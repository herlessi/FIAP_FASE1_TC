...
export default class UserUpdateService implements IService<UserCostumer,Promise<Array<Object>>>{
    ...
    async execute(data:UserCostumer): Promise<Array<Object>> {
        ...
        return this.repo.update({id:data.id,name:data.name,email:data.email,password:data.password})
